...
from .models import product,CartItem,ProductVariation,inc,clintsemails,DiscountCode
from .models import UserActivity
from .filters import ProductFilter
from .forms import SignUpForm ,order,DiscountCodeForm
...
def checkout(requset):
    ...
    discount_code = requset.session.get('discount_code')
    discount_percent = requset.session.get('discount_percent', 0)
    
    if discount_code:
        # Apply the discount to the total price
        total_price = Decimal(total_price) # Your function to calculate the total price
        ...
        if form.is_valid():
            ...
            try:
                ...
                if discount_code.is_valid():
                    # Apply the discount code logic
                    request.session['discount_code'] = discount_code.code
                    request.session['discount_percent'] = discount_code.discount_percent
                    return redirect('check')  # Redirect to your checkout page or any other page
                else:
                    form.add_error('code', 'This discount code is not valid or has expired.')
            except DiscountCode.DoesNotExist:
                form.add_error('code', 'This discount code does not exist.')
    else:
        form = DiscountCodeForm()
    total_price = sum(item.ProductVariation.discount * item.quantity for item in cart_items)
    return render(request, 'html/cart.html', {'cart_items': cart_items, 'total_price': total_price,'form': form})
 #___________________________________________________________________________________
...
def email(requset):
       if requset.method == 'POST':
        email = json.loads(requset.body).get('email')
        print (email)
        # Process the email as needed
        clintsemails.objects.create(email=email)
        ...
        if form.is_valid():
            ...
            try:
                ...
                if discount_code.is_valid():
                    ...
                    request.session['discount_code'] = discount_code.code
                    request.session['discount_percent'] = discount_code.discount_percent
                    return redirect('checkout')  # Redirect to your checkout page or any other page
                else:
                    form.add_error('code', 'This discount code is not valid or has expired.')
            except DiscountCode.DoesNotExist:
                form.add_error('code', 'This discount code does not exist.')
